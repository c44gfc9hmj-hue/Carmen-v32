...
console.log('--- Drea Morgan × Bondage hard regression ---');
{
  ...
  assert((PRIMARY_DIVE_LENSES.map(l => l.id).join(',') === 'bondage,visuals,accounts' || PRIMARY_DIVE_LENSES.map(l => l.id).join(',') === 'bondage,people,visuals'), 'three primary lenses are Bondage / People / Visuals');
}
