...
console.log('--- v47.3 investigation choices adapt by type ---');
{
  ...
  assert(personOn.some(x => x.id === 'visuals' && x.label === 'Visuals'), 'Visuals is a primary Deep Dive path');
  assert(personOn.some(x => x.id === 'clothing' && x.label === 'Clothing'), 'Clothing remains an internal investigation choice');
...
console.log('--- v47.3 UI hides engine internals on the primary path ---');
{
  ...
  assert(!/id="homeAdultChips"/.test(html) && !/id="adultChips"/.test(html), 'Adult Off/On/Both choice is removed');
  assert(/data-testid="photo-input"/.test(html), 'home photo input exists');
...
console.log('--- v47.8 video ID preservation ---');
{
  ...
  uniqueAdd(results, seen, { title: 'Public interview video', url: 'https://www.youtube.com/watch?v=abcDEF12345&feature=share', source: 'Bing Videos' });
  assert(results[0] && results[0].url.indexOf('youtube.com/watch?v=abcDEF12345') >= 0, 'uniqueAdd keeps the real YouTube URL');
  assert(results[0].videoId === 'yt:abcDEF12345', 'uniqueAdd records canonical video ID separately');
  assert(!/^yt:/.test(results[0].url), 'uniqueAdd does not store yt: as the result URL');
  uniqueAdd(results, seen, { title: 'Another public video', url: 'https://www.youtube.com/watch?v=zzzzzzzzzzz', source: 'Bing Videos' });
  assert(results.length === 2, 'distinct video IDs stay distinct in uniqueAdd');
  const vids = collectDiveVideos([], results, { type: 'person', adultContent: 'on', subject: 'Jordan Hale', context: 'adult content' });
  assert(vids.some(v => v.videoId === 'yt:abcDEF12345'), 'adult ON still keeps public YouTube videos');
  assert(vids.some(v => /watch\?v=abcDEF12345/.test(v.url)), 'video corpus URLs remain watchable youtube URLs');
  const vq = videoQueryVariants({ type: 'person', subject: 'Jordan Hale', adultContent: 'on' }, {});
