...
{
  assert(PLANNER_VERSION === '49.9' || PLANNER_VERSION === '49.8' || PLANNER_VERSION === '49.7' || PLANNER_VERSION === '49.6' || PLANNER_VERSION === '49.5' || PLANNER_VERSION === '49.4' || PLANNER_VERSION === '49.3' || PLANNER_VERSION === '49.2', 'PLANNER_VERSION current');

  assert(/49\.(2|3|4|5|6|7|8|9)/.test(PLANNER_BUILD), 'PLANNER_BUILD');
  assert(/49\.[23456789]/.test(readFileSync(new URL('./VERSION', import.meta.url), 'utf8')), 'VERSION file');
  assert(/const VERSION = '49\.[23456789]'/.test(appSrc), 'frontend VERSION');

  assert(/carmen-build" content="49\.[23456789]"/.test(html), 'html build');
  assert(/carmen-v49\.[23456789]/.test(readFileSync(new URL('./public/sw.js', import.meta.url), 'utf8')), 'sw cache');
...
console.log('--- v49.2 health ---');
{
  ...
  const body = await res.json();
  assert(body.version === '49.9' || body.version === '49.8' || body.version === '49.7' || body.version === '49.6' || body.version === '49.5' || body.version === '49.4' || body.version === '49.3' || body.version === '49.2', 'health version');
  assert(/49\.(2|3|4|5|6|7|8|9)/.test(body.build || ''), 'health build');
