...
{
  assert(PLANNER_VERSION === '50' || PLANNER_VERSION === '49.14' || PLANNER_VERSION === '49.13' || PLANNER_VERSION === '49.12' || PLANNER_VERSION === '49.11' || PLANNER_VERSION === '49.9' || PLANNER_VERSION === '49.8' || PLANNER_VERSION === '49.7' || PLANNER_VERSION === '49.6' || PLANNER_VERSION === '49.5' || PLANNER_VERSION === '49.4' || PLANNER_VERSION === '49.3' || PLANNER_VERSION === '49.2', 'PLANNER_VERSION current');

  assert(/50-agent-testable-progressive|49\.(?:2|3|4|5|6|7|8|9|11|12|13|14)/.test(PLANNER_BUILD), 'PLANNER_BUILD');
  assert(/^(50|49\.(?:[2-9]|11|12|13|14))\s*$/.test(readFileSync(new URL('./VERSION', import.meta.url), 'utf8')), 'VERSION file');
  assert(/const VERSION = '(?:50|49\.(?:[2-9]|11|12|13|14))'/.test(appSrc), 'frontend VERSION');

  assert(/carmen-build" content="(?:50|49\.(?:[2-9]|11|12|13|14))"/.test(html), 'html build');
  assert(/carmen-v(?:50|49\.(?:[2-9]|11|12|13|14))/.test(readFileSync(new URL('./public/sw.js', import.meta.url), 'utf8')), 'sw cache');
  ...
  assert(/data-findmore="different"|data-visual-act="different"/.test(appSrc), 'Find different is a distinct control');
...
console.log('--- v49.2 health ---');
{
  ...
  const body = await res.json();
  assert(body.version === '50' || body.version === '49.14' || body.version === '49.13' || body.version === '49.12' || body.version === '49.11' || body.version === '49.9' || body.version === '49.8' || body.version === '49.7' || body.version === '49.6' || body.version === '49.5' || body.version === '49.4' || body.version === '49.3' || body.version === '49.2', 'health version');
  assert(/50-agent-testable-progressive|49\.(2|3|4|5|6|7|8|9|11|12|13|14)/.test(body.build || ''), 'health build');
