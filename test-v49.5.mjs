...
{
  assert(PLANNER_VERSION === '49.9' || PLANNER_VERSION === '49.8' || PLANNER_VERSION === '49.7' || PLANNER_VERSION === '49.6' || PLANNER_VERSION === '49.5', 'PLANNER_VERSION current');
  assert(/49\.9-identity-queue-visual|49\.8-adaptive-investigation|49\.7-retrieval-engine|49\.6-correctness-ux|49\.5-adult-first-nl/.test(PLANNER_BUILD), 'PLANNER_BUILD');
  assert(/const VERSION = '49\.[56789]'/.test(appSrc), 'frontend VERSION');
  assert(appSrc.includes("let currentAdult = 'on'"), 'adult-first default in frontend');
  assert(/carmen-build" content="49\.[56789]"/.test(html), 'html build');
