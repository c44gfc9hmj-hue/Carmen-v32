...
{
  assert(PLANNER_VERSION === '50' || PLANNER_VERSION === '49.14' || PLANNER_VERSION === '49.13' || PLANNER_VERSION === '49.12', 'PLANNER_VERSION 49.12');
  assert(PLANNER_BUILD === '50-agent-testable-progressive' || PLANNER_BUILD === '49.14-person-image-results' || PLANNER_BUILD === '49.13-investigation-actions' || PLANNER_BUILD === '49.12-investigation-workflow', 'PLANNER_BUILD');
  assert(appSrc.includes("const VERSION = '50'") || appSrc.includes("const VERSION = '49.14'") || appSrc.includes("const VERSION = '49.13'") || appSrc.includes("const VERSION = '49.12'"), 'frontend VERSION');
  assert(/carmen-build" content="(50|49\.(12|13|14))"/.test(html), 'html build');
  assert(/v50|v49\.(12|13|14)/.test(html), 'header shows version');
...
console.log('--- health ---');
{
  ...
  assert((body.version === '50' || body.version === '49.14' || body.version === '49.13' || body.version === '49.12') && (body.build === '50-agent-testable-progressive' || body.build === '49.14-person-image-results' || body.build === '49.13-investigation-actions' || body.build === '49.12-investigation-workflow'), 'health reports current');
...
console.log('--- source files do not invent a parallel engine ---');
{
  ...
  assert(/50-agent-testable-progressive|49\.(14-person-image-results|13-investigation-actions|12-investigation-workflow)/.test(plannerSrc), 'planner build marker');
