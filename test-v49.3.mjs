...
{
  assert(PLANNER_VERSION === '50' || PLANNER_VERSION === '49.14' || PLANNER_VERSION === '49.13' || PLANNER_VERSION === '49.12' || PLANNER_VERSION === '49.11' || PLANNER_VERSION === '49.9' || PLANNER_VERSION === '49.8' || PLANNER_VERSION === '49.7' || PLANNER_VERSION === '49.6' || PLANNER_VERSION === '49.5' || PLANNER_VERSION === '49.4' || PLANNER_VERSION === '49.3', 'PLANNER_VERSION current');
  assert(/50-agent-testable-progressive|49.(?:3|4|5|6|7|8|9|11|12|13|14)/.test(PLANNER_BUILD), 'PLANNER_BUILD');
  assert(/const VERSION = '(?:50|49\.(?:[3-9]|11|12|13|14))'/.test(appSrc), 'frontend VERSION');
  assert(/carmen-build" content="(?:50|49\.(?:[3-9]|11|12|13|14))"/.test(html), 'html build');
  ...
  assert((/data-findmore="similar"/.test(appSrc) || /data-visual-act="similar"/.test(appSrc) || /lightboxSimilar/.test(appSrc)) && (/data-findmore="visual"/.test(appSrc) || /lightboxSearchVisual/.test(appSrc) || /searchThisVisual/.test(appSrc)) && (/data-findmore="topic"/.test(appSrc) || /moreOnThisTopic/.test(appSrc)), 'Find similar / Search this visual / More on this topic controls');
...
console.log('--- v49.3 /api docs + health ---');
{
  ...
  assert((h.version === '50' || h.version === '49.14' || h.version === '49.13' || h.version === '49.12' || h.version === '49.11' || h.version === '49.9' || h.version === '49.8' || h.version === '49.7' || h.version === '49.6' || h.version === '49.5' || h.version === '49.4' || h.version === '49.3') && /50|49.(3|4|5|6|7|8|9|11|12|13|14)/.test(h.build || ''), 'health current');
