...
{
  assert(PLANNER_VERSION === '49.9' || PLANNER_VERSION === '49.8' || PLANNER_VERSION === '49.7', 'PLANNER_VERSION current');
  assert(PLANNER_BUILD === '49.9-identity-queue-visual' || PLANNER_BUILD === '49.8-adaptive-investigation' || PLANNER_BUILD === '49.7-retrieval-engine', 'PLANNER_BUILD');
  assert(/const VERSION = '49\.[789]'/.test(appSrc), 'frontend VERSION');
  assert(/carmen-build" content="49\.[789]"/.test(html), 'html build');
  assert(/v49\.[789]/.test(html), 'header shows current version');
...
console.log('--- health features ---');
{
  ...
  assert((body.version === '49.9' || body.version === '49.8' || body.version === '49.7') && /49\.(9-identity-queue-visual|8-adaptive-investigation|7-retrieval-engine)/.test(body.build || ''), 'health reports current');
