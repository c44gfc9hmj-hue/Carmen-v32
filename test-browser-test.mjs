...
console.log('--- browser-test surface ---');
{
  ...
  assert(h.version === PLANNER_VERSION && h.build === PLANNER_BUILD, 'health still reports current version');
...

{
  ...
  assert(html.includes('diveVisualsBtn') && html.includes('Visuals'), '/test HTML has Visuals control');
  assert(!html.includes('diveClothingBtn'), '/test HTML does not expose Clothing as a top-level control');
  ...
  assert(count(html, 'data-testid="dive-visuals"') === 1, 'exactly one dive-visuals testid');
  assert(count(html, 'data-testid="photo-input"') === 1, 'exactly one photo-input testid');
...

{
  ...
  assert(asset.status === 200 && /const VERSION = '(?:50|49\.(?:[4-9]|11|12|13|14))'/.test(js), '/test/app.js is the real frontend');
