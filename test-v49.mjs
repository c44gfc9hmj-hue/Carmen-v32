...
{
  assert(/49\.[3456]/.test(readFileSync(new URL('./VERSION', import.meta.url), 'utf8')), 'VERSION current');
  assert(/const VERSION = '49\.[0-9]'/.test(appSrc), 'frontend VERSION 49.x');

  assert(/carmen-build" content="49\.[3456]"/.test(html), 'html build current');
...
console.log('--- v49 diveSeedQuery keeps subject ---');
{
  ...
  assert(diveSeedQuery(cls, 'Drea Morgan', 'Drea Morgan', '') === 'Drea Morgan', 'bare original name is not rewritten');
}
...
console.log('--- v49 health ---');
{
  ...
  const body = await res.json();
  assert(body.version === '49.6' || body.version === '49.5' || body.version === '49.4' || body.version === '49.3', 'health version current');
  assert(/49\.(3|4|5|6)/.test(body.build || ''), 'health build current');
