...
- **Subject + topic evidence are independent.** A Drea Morgan profile is subject
  evidence. A generic bondage glossary is topic evidence. Strong intersection
  requires both. A query string in the title is never enough.
- **Find everything** builds a dynamic topic map (identity, creator-owned,
  platforms, premium, stores, fetish publishers, studios, video, images,
  community, Reddit, directories, interviews, collaborators, related sites) and
  organizes results by source class instead of one flat wall.
- **Adult source classes** are first-class when Adult Lens is ON: OnlyFans,
  Fansly, LoyalFans, ManyVids, FanCentro, Clips4Sale, Patreon, specialist BDSM
  publishers (including House of Gord), and normal web discovery for new domains.
  Inaccessible sources stay leads — they are not “doesn’t exist.”
- **Premium accounts** expand across subscription platforms and classify
  ownership as creator-owned / official / directory listing / fan-reposter /
  mirror / unknown. A directory mention is never proof of ownership.
- **Known entities** are sites/orgs, never a hardcoded person list. House of
  Gord resolves to houseofgord.com and expands into that ecosystem.
- **Find More / More Like This / Find Different** are distinct intents.
- **Merge, don’t replace.** New runs canonicalize URLs and keep prior evidence.
- **Provenance** keeps host, publisher, creator, originalSource, reposter, and
  mirror distinct. Unknown stays UNKNOWN.
- **Reddit search pages are not Reddit evidence.** If no actual posts/comments
  are retrieved, Reddit is reported unavailable.
- **Competing identity candidates** for genuinely ambiguous names. That’s the
  one / Not this person influence later retrieval.
- **Hard New Investigation** clears live subject, topic, candidates, visuals,
  rejections, trail, and in-flight work. Saved collections stay.
- **Analyze / Teach Me** accept webpage, Reddit, image, and video evidence
  objects. Video frames/timestamps are UNKNOWN until real frame analysis exists.
- **Surprise me** is a bounded heuristic over explicit saves/selections — it
  says so.

Build identifier: **49.2-topic-map-retrieval**.
